...
# 🔗 OneDrive/SharePoint link (ensure public access)
...
if not xls:
    st.error("❌ Failed to load Excel file from OneDrive.")
else:
    ...
    if selected_sheets:
        ...
        status_filter = st.sidebar.multiselect("✅ Filter by Status", combined_df["Status"].dropna().unique(), default=combined_df["Status"].dropna().unique())
        health_filter = st.sidebar.multiselect("🩺 Filter by Health", combined_df["Health"].dropna().unique(), default=combined_df["Health"].dropna().unique())
        manager_filter = st.sidebar.multiselect("👤 Filter by Project Manager", combined_df["Project Manager"].dropna().unique(), default=combined_df["Project Manager"].dropna().unique())
        ...
        st.download_button("⬇️ Download Filtered Data", data=csv, file_name="filtered_projects.csv", mime='text/csv')
...
if not os.path.exists(excel_file):
    ...
    # ✅ Include only May and June; exclude April Demo and Skill Set
